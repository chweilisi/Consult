...
//                                GONE=1000 PERCEPTIBLE=130 SERVICE=300 ISIBLE=20
    ...
    public static boolean isNetWorkAvailable() {
        return isNetWorkAvailable((App)getApplication());
    }
...
//                                因为有可能是多音字
    ...
    public static List<String> string2list(String string) {
        if (string != null && string.contains(",")) {
            LogUtils.e(TAG, "这里内容需要转义");
            return new ArrayList<>(Arrays.asList(string.split(",")));
        } else if (string != null) {
            LogUtils.e(TAG, "只有一个字符串，没有分隔符");
            List<String> list = new ArrayList<>();
            list.add(string);
            return list;
        } else {
            LogUtils.e(TAG, "string中为空");
            return null;
        }
    }
    ...
    public static List<String> commentMsg2List(String commentMessage) {
        if (commentMessage != null && commentMessage.contains("&")) {
            ...
            LogUtils.e(TAG, "只有一个评论的情况下");
            List<String> list = new ArrayList<>();
            list.add(commentMessage);
            return list;
        } else {
            LogUtils.e(TAG, "评论消息为空");
            return null;
        }
    }
